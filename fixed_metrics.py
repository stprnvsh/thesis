#!/usr/bin/env python3
"""
Corrected Metrics for Hawkes Process Models
"""
...
def geweke_diagnostic(samples, first_frac=0.1, last_frac=0.5):
    ...
    # Data structure is (samples, ...) not (chains, samples, ...)
    n_samples = samples.shape[0]
    first_n = int(n_samples * first_frac)
    last_n = int(n_samples * last_frac)
    
    z_scores = []
    for i in range(min(5, samples.shape[1])):  # Test first 5 params
...
def monte_carlo_se(samples):
    ...
    for i in range(min(5, samples.shape[1])):
        ...
        # MCSE = std / sqrt(n_samples) for independent samples
        # Since we have autocorrelation, use effective sample size
        acf = np.correlate(param_samples, param_samples, mode='full')
        acf = acf[acf.size//2:acf.size//2+20]  # First 20 lags
        acf = acf / acf[0]
...
def assess_fit_quality(state_file):
    """Assess fit using corrected metrics"""
    print(f"\n{'='*60}")
...
def main():
    ...
    print("CORRECTED METRICS FOR HAWKES PROCESS MODELS")
    print("=" * 60)
    print("Data structure: (samples, ...) not (chains, samples, ...)")
