#!/usr/bin/env python3
"""
Final Corrected Metrics for Hawkes Process Models
"""
...
def geweke_diagnostic_3d(samples, first_frac=0.1, last_frac=0.5):
    """Geweke diagnostic for 3D samples (samples, dim1, dim2)"""
    if samples.ndim != 3:
        return np.nan
    ...
    z_scores = []
    # Test a few representative parameters
    test_indices = [
        (0, 0), (0, 1), (1, 0), (1, 1), 
        (samples.shape[1]//2, samples.shape[2]//2)
    ]
    
    for i, j in test_indices:
        if i < samples.shape[1] and j < samples.shape[2]:
            param_samples = samples[:, i, j]
...
def heidelberger_welch_3d(samples):
    """Heidelberger-Welch for 3D samples"""
    if samples.ndim != 3:
        return np.nan
...
def effective_sample_ratio_3d(samples):
    """ESS ratio for 3D samples"""
    if samples.ndim != 3:
        return np.nan
    ...
    for i, j in test_indices:
        if i < samples.shape[1] and j < samples.shape[2]:
            ...
            acf = acf[acf.size//2:acf.size//2+20]
            acf = acf / acf[0]
...
def monte_carlo_se_3d(samples):
    """Monte Carlo SE for 3D samples"""
    if samples.ndim != 3:
        return np.nan
    ...
    for i, j in test_indices:
        if i < samples.shape[1] and j < samples.shape[2]:
            ...
            ess = len(param_samples) / (1 + 2 * np.sum(acf[1:lag+1]))
            mcse = np.std(param_samples) / np.sqrt(ess) if ess > 0 else np.nan
            mcse_values.append(mcse)
    
    return np.array(mcse_values)

def assess_fit_quality(state_file):
    """Assess fit using properly corrected metrics"""
    print(f"\n{'='*60}")
    ...
    try:
        ...
        for param_name in key_params:
            if param_name in state:
                ...
                if samples.ndim == 3:
                    # Compute all metrics for 3D data
                    geweke = geweke_diagnostic_3d(samples)
                    heidel = heidelberger_welch_3d(samples)
                    mcse = monte_carlo_se_3d(samples)
                    ess_ratio = effective_sample_ratio_3d(samples)
                    ...
                    print(f"  Scalar parameter - no diagnostics")
...
def main():
    ...
    print("FINAL CORRECTED METRICS FOR HAWKES PROCESS MODELS")
    print("=" * 60)
    print("Data structure: (samples, dim1, dim2)")
